    ...
    if os.path.isfile("download.yaml"):
        ...
        with open("download.yaml", "r") as yf:
            try:
                ...
                logging.info(
                    "Special path not found in download.yaml and will not be used"
                )
    else:
    ...
    if os.path.isfile("download.json"):
        ...
        with open("download.json", "r") as jf:
            try:
                ...
                logging.info(
                    "Special path not found in download.json and will not be used"
                )
    else:
...
def main():
    ...
    if special_path is None:
        logging.info(
            "It does not appear that a special image path was specified. Using default image path value."
        )
        return 0
    else:
