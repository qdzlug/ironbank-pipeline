...
def vulnerability_report(csv_dir, anchore_security_json, justifications):
    ...
    for vuln in vulns:
        ...
        vuln_dict = {k: v for k, v in vuln.dict().items() if k in fieldnames}
        # get justification if exists
        vuln_dict["Justification"] = AnchoreSecurityParser.get_justification(
            vuln=vuln, justifications=justifications
        )
