...
class ParseCsvsTestCase(unittest.TestCase):
   ...
   def test_parse_csvs(self):
      ...
      assert tlc['description'][0] == 'Apache Commons FileUpload before 1.3.3 DiskFileItem File Manipulation Remote Code Execution', "description"
      assert tlc['severity'][0] == 'critical', "severity = critical"
      assert tlc['link'][0] == 'https://web.nvd.nist.gov/view/vuln/detail?vulnId=CVE-2016-1000031', "link"
      assert tlc['package'][0] == 'commons-fileupload_commons-fileupload-1.3.1-jenkins-2', "package"
      assert tlc['package_path'][0] == 'N/A', "package_path = "


      asc = rslt['anchore_cve']
      assert asc.at[0,'finding'] == 'CVE-2019-9948', "finding"
      assert asc.at[0,'description'] == "Python-2.7.5\nhttps://nvd.nist.gov/vuln/detail/CVE-2019-9948", "description"
      assert asc.at[0,'link'] is None, "link"
      assert asc.at[0,'package'] == "Python-2.7.5", "package"
      assert asc.at[0,'package_path'] == "/opt/app-root/lib/python3.6/site-packages/pip", "package"
      ...
      assert occ.at[137,'description'] == 'Verify Any Configured IPSec Tunnel Connections', "description"
