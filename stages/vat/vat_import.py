...
parser.add_argument("-pc", "--parent", help="Parent VENDOR/PRODUCT/CONTAINER", default=None, required=False)
parser.add_argument(
    "-pv", "--parent_version", help="Parent Version from VENDOR/PRODUCT/CONTAINER/VERSION format", default=None, required=False
)
  ...
  """
    try:
        ...
        if args.jenkins is not None and args.scan_date is not None:
            ...
            logs.warning("scan date either can not not computed or is older : check params")
    except Error as error:
        logs.error(error)
    finally:
        if conn is not None and conn.is_connected():
