...
echo "${profile}"
echo "${securityGuide}"
securityGuide=$(echo "${securityGuide}" | sed "s/{OSCAP_VERSION}/${OSCAP_VERSION}/g")
export securityGuide
{ SUCCESS=0; oscap-podman "${DOCKER_IMAGE_PATH}" xccdf eval --verbose ERROR --fetch-remote-resources --profile "${profile}" --report report.html "${securityGuide}" || SUCCESS=$?; }
ls report.html
