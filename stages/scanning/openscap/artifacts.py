...
class Artifacts(LoggerMixin):
    ...
    def _remove_scap_content(self) -> None:
        ...
        try:
            ...
            self._log.info(f"Directory {scap_content_dir_str} removed.")
        except OSError as exc:
            self._log.warning(f"The {scap_content_dir} was not removed: {exc}")

    def _save_oscap_version(self, oscap_version: str) -> None:
        self._log.info(f"OpenSCAP version: {oscap_version}.")
    ...
    def _move_reports(self) -> None:
        ...
        for file_name in files_to_copy:
            ...
            if source_file.is_file():
                ...
                self._log.info(f"File {source_file} moved to {destination_file}.")
            else:
                self._log.warning(f"{file_name} does not exist.")

    def _save_job_url(self) -> None:
        env_path: Path = Path("oscap-compliance.env")
        ci_job_url = Envs().ci_job_url
        self._log.info(ci_job_url)
