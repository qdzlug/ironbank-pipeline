...
parser.add_argument("-d", "--db", help="Connection Info: Database to connect to", required=True)
...
def get_containerId(conn):
    cursor = conn.cursor()
    cursor.execute("SELECT id FROM containers WHERE name = %s AND version = %s", (args.image, args.tag))
    row = cursor.fetchone()
...
def verify_approvalStatus(conn):
    ...
    cursor.execute("SELECT imageId, MAX(date_time) FROM container_log WHERE imageid = %s GROUP BY imageId", (containerId,))
    row = cursor.fetchone()
    ...
    cursor.execute("SELECT type, user_id FROM container_log WHERE date_time = %s", (approved_date,))
    row = cursor.fetchone()
...
def create_greylist(conn, approvalStatus, approved_date, user_id):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT id, finding, scan_source, package FROM findings_approvals WHERE imageId = %s", (containerId,))
    ...
    for row in rows:
        ...
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT type, MAX(date_time) FROM findings_log WHERE approval_id = %s AND type = 'Justification' GROUP BY type", (approval_id,))
        row = cursor.fetchone()
        # If there is a justification, write data to json
        if row:
            approved_date = (row['MAX(date_time)'])
            justificationDateTime = row['MAX(date_time)']
            cursor.execute("SELECT text FROM findings_log WHERE date_time = %s AND approval_id = %s", (justificationDateTime, approval_id))
            row = cursor.fetchone()
